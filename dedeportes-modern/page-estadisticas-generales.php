 ...
 */
if (!function_exists('get_tournament_stats')) {
...
/**
 * Función auxiliar para renderizar una tarjeta de Top 5 (Diseño v2.24)
 */
if (!function_exists('render_top5_card')) {
    function render_top5_card($title, $data, $metric_key, $metric_label)
    {
        if (empty($data))
            return;
        echo '<div class="sidebar-widget top5-card" style="padding: 0; overflow: hidden; border: 1px solid var(--border); box-shadow: var(--shadow-md); display: flex; flex-direction: column; background: var(--card-bg); border-radius: var(--radius-md); margin-bottom: 2rem;">';
...
if (!function_exists('is_chilean_team')) {
    function is_chilean_team($team)
    {
        ...
        foreach ($chilean_patterns as $p) {
            if (stripos($team['Equipo'], $p) !== false)
                return true;
        }
        return false;
    }
